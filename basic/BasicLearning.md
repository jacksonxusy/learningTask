...
```
address           // 普通地址，不能发送 ETH
address payable   // 可支付地址，能接收 ETH
```
diff:
```
address addr = 0x123...;
addr.transfer(1 ether);  // ❌ 编译错误！address 没有 transfer 方法

address payable pAddr = payable(0x123...);
pAddr.transfer(1 ether);  // ✅ 可以
```
