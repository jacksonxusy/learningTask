...
│                                                                                             │
│  ┌─────────┐    ┌─────────┐    ┌─────────────┐    ┌───────────────┐    ┌────────────────┐  │
│  │  USER   │───▶│FRONTEND │───▶│   WALLET    │───▶│  BLOCKCHAIN   │───▶│  SYNC SERVICE  │  │
│  │(Browser)│    │(Next.js)│    │ (MetaMask)  │    │ (Smart Contract)│  │ (Go Indexer)   │  │
│  └─────────┘    └─────────┘    └─────────────┘    └───────────────┘    └────────────────┘  │
│       │              │               │                   │                    │            │
│       │  Click List  │               │                   │                    │            │
│       │─────────────▶│               │                   │                    │            │
...
📦 Service 1: EasySwapSync (Indexer)
Location: /EasySwapSync

Purpose: Listen to blockchain events and index them to database

Key Points:

❌ Does NOT listen to raw transactions
✅ Listens to smart contract events (LogMake, LogMatch, LogCancel)
✅ Polls every 10 blocks (configurable)
✅ Parses structured event data
✅ Writes to MySQL + Redis
Example Flow:

go
// Continuously polls blockchain
for {
    logs := chainClient.FilterLogs(startBlock, endBlock)
    
    for _, log := range logs {
        switch log.Topics[0] {
        case LogMakeTopic:
            handleMakeEvent(log)  // Parse & save to DB
        case LogMatchTopic:
            handleMatchEvent(log) // Update order status
        }
    }
}
🌐 Service 2: EasySwapBackend (API)
Location: /EasySwapBackend

Purpose: Serve data to frontend via REST API

Key Points:

✅ Gin web framework
✅ Reads from MySQL (no blockchain calls)
✅ Fast queries (indexed database)
✅ REST endpoints for frontend
Example Endpoints:

GET  /api/v1/orders?collection=0xCoolCats...&status=active
GET  /api/v1/collections/0xCoolCats.../stats
GET  /api/v1/activities?type=sale&limit=10
GET  /api/v1/items/0xCoolCats.../42
🤔 Why Two Services?
Why not query blockchain directly from frontend?
Approach	Speed	Cost	Features
Direct blockchain query	🐌 Slow (RPC calls)	💰 Expensive	❌ No filtering, sorting
Backend API (this project)	⚡ Fast (MySQL)	💸 Cheap	✅ Complex queries, pagination
Benefits of this architecture:
Speed: Database queries are 100x faster than blockchain RPC calls
Features: Can do complex queries (filtering, sorting, aggregations)
Reliability: Frontend doesn't depend on blockchain RPC uptime
UX: Users see instant results, no loading spinners


### service.go
#### SyncOrderBookEventLoop
┌─────────────────────────────────────────────────────────────┐
│              SyncOrderBookEventLoop Flow                     │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  1. Query DB: What block did we last sync?                  │
│     → lastSyncBlock = 1040                                  │
│                                                              │
│  2. Query Blockchain: What's the current block?             │
│     → currentBlockNum = 1100                                │
│                                                              │
│  3. Calculate range to sync (safe from reorgs):            │
│     → startBlock = 1040                                     │
│     → endBlock = 1040 + 10 = 1050 (batch of 10)            │
│                                                              │
│  4. Fetch events from blocks 1040-1050:                     │
│     → FilterLogs(1040, 1050, OrderBookAddress)             │
│                                                              │
│  5. Process each event:                                     │
│     ┌──────────────────────────────────────┐                │
│     │ LogMake   → handleMakeEvent()        │                │
│     │ LogMatch  → handleMatchEvent()       │                │
│     │ LogCancel → handleCancelEvent()      │                │
│     └──────────────────────────────────────┘                │
│                                                              │
│  6. Update DB: lastSyncBlock = 1051                         │
│                                                              │
│  7. Sleep 10 seconds                                        │
│                                                              │
│  8. Loop back to step 2                                     │
│     (next iteration: sync blocks 1051-1061)                 │
│                                                              │
└─────────────────────────────────────────────────────────────┘
